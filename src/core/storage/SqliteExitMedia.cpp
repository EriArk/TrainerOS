...
QList<ExitMedia> readExitMedia(QSqlDatabase& db) {
    ...
    if (!q.exec("SELECT m.session_id,m.trainer_id,m.domain,m.adventure_id,m.registration_revision,m.build_sha256,m.captured_at,m.jpeg,m.image_sha256,m.content_path,m.content_size,m.content_modified,m.width,m.height FROM exit_media m JOIN trainer_profile p ON p.id=m.trainer_id AND p.slot=1 JOIN adventures a ON a.id=m.adventure_id AND a.revision=m.registration_revision AND a.content_path=m.content_path JOIN play_sessions s ON s.id=m.session_id AND s.adventure_id=m.adventure_id AND s.outcome='returned' WHERE length(m.jpeg) BETWEEN 1 AND 524288 ORDER BY s.rowid DESC LIMIT 100")) return result;
