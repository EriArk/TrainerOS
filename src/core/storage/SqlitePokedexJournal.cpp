...
PokedexJournalSnapshot readPokedexJournal(QSqlDatabase& db, const QString& owner) {
    PokedexJournalSnapshot snapshot;QSqlQuery q(db);
    q.prepare("SELECT entry_id,seen,caught,notes,revision FROM pokedex_records WHERE trainer_id=?");q.addBindValue(owner);
    if (!q.exec()) return {{},"Your field journal couldn't be read. Existing records have been kept."};
    while(q.next()) {
...
PokedexWriteResult writePokedexRecord(QSqlDatabase& db, const QString& owner, const QString& id, const PokedexProgress& record, PokedexJournalSnapshot& snapshot) {
    ...
    QString error;
    {
        ...
        q.prepare(creating ? "INSERT INTO pokedex_records(seen,caught,notes,revision,entry_id,trainer_id) VALUES(?,?,?,?,?,?)"
            : "UPDATE pokedex_records SET seen=?,caught=?,notes=?,revision=? WHERE entry_id=? AND trainer_id=? AND revision=?");
    ...
    if(error.isEmpty()){snapshot=readPokedexJournal(db, owner);error=snapshot.error;}
