...
Item {
    ...
    readonly property var cardPalette: Worlds.theme(entry.id)
    ...
    Canvas {
        ...
        onPaint: {
            ...
            edge(1);c.fillStyle=Qt.darker(root.cardPalette[0],1.65);c.fill()
            c.save();edge(3);c.clip()
            const wash=c.createLinearGradient(0,0,0,h)
            wash.addColorStop(0,Qt.lighter(root.cardPalette[0],1.06));wash.addColorStop(1,root.cardPalette[0]);c.fillStyle=wash;c.fillRect(0,0,w,h)
            const colors=Worlds.scenery(root.cardPalette[2]);var colorIndex=0
            c.scale(w/280,h/150);c.strokeStyle=colors[0];c.fillStyle=colors[0];c.lineWidth=3;c.globalAlpha=.85
            function line(points,fill) {
                c.fillStyle=colors[colorIndex%colors.length];c.strokeStyle=colors[colorIndex++%colors.length]
            ...
            const motif=root.cardPalette[2]
            if(motif==="mountains") {
                ...
                for(let y=20;y<170;y+=26){c.strokeStyle=colors[colorIndex++%colors.length];c.beginPath();c.moveTo(-10,y);c.bezierCurveTo(60,y-38,98,y+35,155,y);c.bezierCurveTo(200,y-27,237,y+28,295,y-8);c.stroke()}
                ...
                for(let i=0;i<8;++i){const x=i*39-10,y=135-heights[i];c.fillStyle=colors[i%4];c.fillRect(x,y,29,heights[i]);c.fillStyle="#f6edc1";for(let j=y+8;j<120;j+=13){c.fillRect(x+6,j,4,4);c.fillRect(x+18,j,4,4)}}
                ...
                if(motif==="shrine") {c.globalAlpha=1;c.lineWidth=5;colorIndex=2;line([[104,113],[104,51],[205,51],[205,112]],false);colorIndex=2;line([[86,39],[104,46],[205,46],[222,36]],false);colorIndex=2;line([[116,70],[194,70]],false)}
            } else if(motif==="leaves") {
                for(let i=0;i<6;++i){c.save();c.fillStyle=colors[i%4];c.translate(i*58-12,80+(i%2)*39);c.rotate(-.8+i*.3);c.beginPath();c.moveTo(0,30);c.bezierCurveTo(-42,0,-22,-52,0,-61);c.bezierCurveTo(31,-27,42,4,0,30);c.fill();line([[0,30],[0,-50]],false);c.restore()}
            } else if(motif==="stadium") {
                c.save();c.translate(153,89);c.scale(1,.48);for(let r=42;r<153;r+=21){c.strokeStyle=colors[colorIndex++%4];c.beginPath();c.arc(0,0,r,0,Math.PI*2);c.stroke()}c.restore()
                for(let i=0;i<7;++i)line([[i*48-20,113],[i*48+6,20],[i*48+34,113]],false)
            } else if(motif==="garden") {
                c.save();c.translate(158,62);for(let i=0;i<10;++i){c.fillStyle=colors[i%4];c.strokeStyle="#f5d8ee";c.rotate(Math.PI/5);c.beginPath();c.moveTo(0,0);c.bezierCurveTo(-35,-37,-10,-74,0,-83);c.bezierCurveTo(28,-57,35,-25,0,0);c.fill();c.stroke()}c.restore()
            ...
            textWash.addColorStop(0,"#00ffffff");textWash.addColorStop(1,Qt.lighter(root.cardPalette[0],1.1))
