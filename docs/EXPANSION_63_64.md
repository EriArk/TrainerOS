...
The issue nominates [SteGriff/pokemon-badges](https://github.com/SteGriff/pokemon-badges), attributing Stephen Griffiths (2011), CC BY 3.0 Unported, and the first 40 badges from Kanto through Unova Black/White 1. At import time verify the source revision, license, actual asset coverage and modification requirements. The initial planning review had not imported or independently reviewed these assets; the first delivery below records the later source inspection and bounded import. Do not describe the underlying Pokemon designs as TrainerOS-owned or assume later games are covered.
...
**Acceptance:** exact-set and ordered-ID mapping, earned/unearned/count-only/unknown states, missing-set and later-title fallback, no cross-generation substitution, preserved SVG aspect/padding, compact Home and large Hall layouts, and included credits. Retire `BadgeCrystal` in normal game-backed surfaces only after replacement/fallback coverage works; temporary development fixtures may retain it. See the bounded delivery checkpoint below; full Journey and in-game visual acceptance are not implied.

## First badge delivery - 2026-09-19

The source README/license and revision were inspected. Sixteen Kanto/Hoenn vector recreations and bounded 192px transparent derivatives now have explicit per-asset IDs, hashes, crop/provenance metadata and controller-accessible Start -> Settings -> Credits attribution. Embedded raster reference images were excluded. [Source, modifications and ordered mapping](../assets/badges/README.md).

The existing verified FireRed (English original/Rev 1) and Emerald providers now publish exact semantic set IDs. Home uses a shared `BadgeTray`/`LeagueBadge` presentation through `BadgeAssets`, with no QML asset-path selection or parser artwork paths. Earned, unearned, unknown and unsupported-set states are separate; a count without resolved identities does not fabricate a sequence. The original crystal component is no longer packaged or used by Home. No additional title/parser, save write or database migration was added.

The same tray renders at a larger ceremonial size in an isolated fixture. This prepares the UI component, **not** a real Hall/Journey consumer: #47 still awaits its later data work. The owner explicitly retained a later in-game comparison of shapes, colors, order and earned state against actual FireRed/Emerald screens. Reference-sheet inspection and gym-flag order checks do not close that acceptance. Later sets remain neutral until their own mapping/art coverage is verified. Issue #64 stays open for those remaining criteria.

Windows: 33/33 tests passed, including resource decode/identity/unknown-state checks and rendered controller scenarios; the affected persistence/render suite passed again after the mipmap adjustment. Compact Home, both sets, zero/unknown/missing-set and large fixture views were inspected, along with controller Credits and focus return. ARM64: 37/37 checks passed (36 plus the separate persistence-process run), followed by a successful non-testing build. Installed on the updated Armada 20260919.17edccf with a prior binary/database backup; SHA-256 `2fff5293251e1ff43ca7ca7d9f98121b40ed04aeee3fd3dbdc756bf225be501f`. Gamescope captures on Flip showed both actual FireRed and Emerald badge sets with 8/8 and 386 caught. InputPlumber controller events exercised Credits, Start/Back restoration, Y selection without launching and L1/R1 return; final Home was restored to FireRed. Database integrity passed, retaining schema 7 and 686 Adventures. No game/save was modified or launched during this visual check. Large Journey rendering remains fixture-only, and the owner-requested in-game artwork comparison remains open.
