...
**Owner UI refinement, 2026-09-23:** remove the normal-use sprite/portrait inspector. Keep per-asset attribution metadata now; add editable author/source/terms fields to Pack Studio and consolidated Credits in the final artwork stage. The illustrated detail companion remains.
...
**Owner amendment, 2026-09-20:** retain #58 -> #60 -> #57 runtime prerequisites; move **#59 Pack Studio to the very end**, after handheld UI and asset structure stabilize. Settings no longer waits for Studio. The numbered items below preserve acceptance, not execution order; [ROADMAP.md](ROADMAP.md) owns the queue. No daemon/resident downloader; retain real Flip profile validation.
...
3. **#57 generic contract:** establish an evolvable versioned ZIP/folder manifest, stable species/form slots, hashes, authors/provenance, originals/transforms/runtime derivatives and profiles from #60. Partial packs are valid. Define exact-form, explicitly compatible primary/secondary fallback and neutral placeholder behavior. Shared validation rejects traversal/absolute paths, bad hashes/images and excessive counts/bytes; activation is atomic and preserves the prior valid pack. No ROM/save writes. Implement loading/validation and install/verify/remove mechanics when a runtime consumer needs them. Settings can proceed without Studio; defer final authoring-format stabilization until UI consumers and structure settle.
4. **#59 Pack Studio (final delivery stage):** a separate Windows/Linux Qt/C++ desktop artist tool, intentionally mouse-friendly; normal use needs no Python, Node or browser. Consume the same validator/schema and Flip-proven inventory/profiles. Provide batch mapping, missing/review/next workflow, nondestructive fit/drag/zoom, immediate slot previews/safe-area warnings, autosave, credits and validate/export/reimport. It must support future original art without depending on #58 at runtime or inventing independent dimensions/form keys.
...
Audit scope: all tracked Markdown, including initial CODEX_START, adapter/session/backup/feasibility documents and Help acceptance. Legacy type names and test records remain only under visible implementation/historical notices; active future tasks no longer expand state resume. Runtime Help does not yet exist. Per the owner's 2026-09-19 clarification, #40 viewer and product articles are deferred to P11 after the main flows stabilize; engineering docs and capability evidence continue to evolve with features. The final guide must describe delivered capabilities and distinguish anything still planned.
