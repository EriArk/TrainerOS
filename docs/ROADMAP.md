...
**Next visible increment:** early P7/#57 generic artwork contract and shared validation, using the completed [real Flip bootstrap and measured profiles](ARTWORK_BOOTSTRAP.md). Preserve its exact identities, private provenance, explicit missing/fallback rules and bounded derivatives; prove staged validation, cancellation and atomic activation before #59 Pack Studio. The 142 unresolved forms remain explicit review work, not invented defaults. #51 sprites remain a separate optional detail/scene provider; production living Party stays late P8. #64 Home is implemented, with later in-game comparison and the real Journey consumer still open. All unfinished P1 UI flows below retain their place; no new save research is introduced.
