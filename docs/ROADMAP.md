...
The first [persistent Hall of Fame increment](HALL_OF_FAME.md) adds manual memory creation/editing, owned-Adventure selection/search, date/time/team/notes entry and revision-checked SQLite storage. Personal runs no longer show fixture archive entries or fake unlocks. The [Trainer overview](TRAINER_OVERVIEW.md) adds local library/journal/archive/time aggregates and a full-reference favorite picker. Real RetroAchievements account/cache integration remains follow-up work; this does not close the entire milestone.
