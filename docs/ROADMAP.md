...
**Next visible increment:** #64 accurate badges for the already verified GBA progress path: review the nominated SVG source/terms and exact set mapping, add the shared asset boundary and neutral states, replace the current Home crystals, include Credits, and inspect compact/large fixture rendering. Do not add save formats to this slice. Pokedex art/sprites are also early UI work: proceed through the #58/#60 gates as their inputs/device become available, not after save integrations. This scheduling authorization does not mean assets have been imported or reviewed yet.
