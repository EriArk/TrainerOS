...
The owner's 2026-09-19 clarification sets Switch-style shell A/B positions: right A confirms, bottom B goes back. On the current Flip InputPlumber Xbox target these are SDL B/east and A/south; `ControllerInput` owns this conversion and diagnostics shows the shell labels. Raw diagnostic exports retain SDL names. UI smoke scenarios inject these raw positions rather than disabling the production mapping. Keyboard Enter/Escape, X/Y and L1/R1 are unchanged. No mappings are hard-coded into feature QML. The first connected supported controller is used; reconnect is supported. A future device-profile/remapping UI is not implemented yet.
