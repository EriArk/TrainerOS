...
- Planned updated #9: one shell-owned Choose Adventure drawer selects a shared per-Trainer `CurrentPokemonAdventureContext` across Pokémon-aware screens; Worlds retains local search/filter controls. No duplicate persistent Current Adventure capsule/chip. Shared Y and the Pokedex/Center and Hall/RA pairs are delivered. Schema 9 now binds owner-isolated personal repositories, real profile creation/selection and TrainerOS RA accounts; PIN/startup gating and additional domain consumers remain planned.
